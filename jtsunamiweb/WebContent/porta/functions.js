...
function loadSubscribers(target,/*element*/ id) {
	...
	http.onreadystatechange = function() {
	    ...
	    if (http.readyState == 4)  {
			...
			try {
				parentElement.innerHTML = "";
				parentElement.innerHTML = "<p>Cargando subscriptores y grupos...</p>";
			} catch (e) {
				wrappingDiv = document.createElement('div');
				wrappingDiv.innerHTML = "<p>Cargando subscriptores y grupos...</p>";
				parentElement.appendChild(wrappingDiv);
			}
		}
	};
...
function addMsisdn(targetO, targetD) {
	...
	for (var i = origen.length - 1; i >= 0; i--) {
		...
		if (origen[i].selected) {
			...
			//origen.options[i].innerHTML = '';
			origen.remove(i);
			
			
		/*	var subsx = document.getElementsByName('msisdnSelectedx')[0];
